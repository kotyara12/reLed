...
Для обеспечения доступа к основному светодиоду из различных модулей и библиотек и индикации режима устройства используйте функции из модуля reLedSys.h. При вызове ledSysInit(...) создается задача ledSystem, и затем все вызовы перенаправляются на нее. Если вызова ledSysInit(...) не было, то команды "уходят в космос".
...
Доступные флаги см. в файле reLedSys:
